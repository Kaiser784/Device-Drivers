...
char chr_array[40]="Welcome to Device Drivers Lab\n";
static int length = 0;
...
static ssize_t read_proc(struct file *filp, char __user *buf, size_t len, loff_t *off);
...
static ssize_t write_proc(struct file *filp, const char *buf, size_t len, loff_t *off){
	...
	copy_from_user(chr_array,buf,40);
	return len;
}
static int release_proc(struct inode *inode, struct file *file){
	printk(KERN_INFO" Procfs File is opened\n");
