...
static int __init chr_driver_init(void)
{
    ...
    dev_t dev = MKDEV(496, 0);
    ret = register_chrdev_region(dev, 1, "my_Dev");
	if (ret < 0) {
		printk(KERN_INFO"err_handle: Test 1: Chrdev allocation failure\n");
	    goto out;
	}
	...
	if (IS_ERR(dev_my)) {
		printk(KERN_INFO"err_handle: Device creation failure\n");
		ret= PTR_ERR(dev_my);
		goto r_device;
	}
